...
export default async function PointsRelaisPage({
  ...
  const qs = new URLSearchParams({ pageSize: "50", role: "relais" });
  ...
  return (
    <>
      <DashboardHeader
        title="Points relais"
        subtitle="Gestion des points relais partenaires"
        session={session}
        rightSlot={
          <form method="GET">
        ...
        <div className="flex flex-col gap-[30px] max-w-[1150px]">
          <div className="flex flex-row flex-wrap items-center justify-between gap-4">
            <div className="flex flex-row rounded-input overflow-hidden border border-figma-tableBorder bg-figma-tableHeader shadow-card">
              ...
              <Tab href="/dashboard/utilisateurs/points-relais" active>Points relais</Tab>
            </div>
            <span className="text-xs text-figma-adminSub">{total} point{total !== 1 ? "s" : ""} relais</span>
          </div>
                ...
                {users.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-10 px-4 text-center text-sm text-figma-adminSub">
                      {q ? `Aucun résultat pour "${q}".` : "Aucun point relais pour le moment."}
                    </td>
                  </tr>
                ) : users.map((u) => {
                  ...
                  const pays = String(profile.country ?? "—");
                  const ville = String(profile.city ?? "—");
                  return (
                    <tr key={u.id} className="border-b border-figma-tableRowBorder last:border-0 bg-white hover:bg-figma-tableRowHover transition-colors duration-fast">
                      ...
                      <td className="py-3 px-4 text-sm text-figma-headerTitle">{ville}</td>
