...
export function PartnerEspaceHub({ role }: { role: Exclude<AppRole, "admin" | "client" | "solupacker"> }) {
  ...
  return (
    <div className="mt-6 grid gap-4 md:grid-cols-2">
      {base ? (
        <Reveal>
          <Link href={base} className="card p-6 focus-ring md:col-span-2">
      ...
      {extras.map((c, idx) => (
        <Reveal key={c.href} delayMs={100 + idx * 70}>
      ...
      <Reveal delayMs={180}>
        <Link href="/notifications" className="card p-6 focus-ring">
      ...
      <Reveal delayMs={250}>
        <Link href="/parametres" className="card p-6 focus-ring">
